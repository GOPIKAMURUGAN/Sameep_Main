...
export function buildPageViewPayload({ pageType, vendorId = "" }) {
...
export function buildSiteAnalyticsPayload({
  pageType,
  eventType,
  vendorId = "",
  meta = {},
...
export function shouldTrackPageViewOnce(key) {
  if (typeof window === "undefined") return false;
  const marker = `siteAnalyticsTracked:${key}`;
  if (window.sessionStorage.getItem(marker)) return false;
  window.sessionStorage.setItem(marker, "1");
  return true;
}

export async function trackSitePageView(payload) {
  return trackSiteEvent(payload);
}
...
export async function trackSiteEvent(payload) {
  ...
  try {
    ...
    console.error("Failed to track site analytics event", error);
  }
}
