...
const CATEGORY_API =
  `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/dummy-categories`;
...
export default function ChooseCategoryModal({ onClose }) {
  ...
  const handleContinueWithoutOtp = async () => {
    ...
    try {
      ...
      const bypassRes = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/customers/bypass-otp`,
        {
          method: "POST",
      ...
      const vendorRes = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/dummy-vendors`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const handleGoogleSearch = async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/google/places/search?query=${encodeURIComponent(
          businessQuery
        )}`
      );
  ...
  const fetchBusinessDetails = async () => {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/google/places/details?placeId=${activePlaceId}`
    );
              ...
              <button
                ...
                onClick={async () => {
                  try {
                    ...
                    await fetch(
                      `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/vendor-price-nodes/sync`,
                      {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({
